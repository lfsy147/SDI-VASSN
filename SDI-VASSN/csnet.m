...
%Construction of cell-specific networks细胞专用网络的构建
%The function performs the transformation from gene expression matrix to
%cell-specific network (csn).该功能执行从基因表达矩阵到细胞专用网络
%data: Gene expression matrix, rows = genes, columns = cellsdata：基因表达矩阵，行=基因，列=细胞
